...
const BlogForm = () => {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div>
        <label htmlFor="isPublished" className={form.formState.errors.thumbnail && 'text-red-500'}>
          انتشار؟
        </label>
        <Controller
          control={form.control}
      ...
      <div>
        <label htmlFor="schedule" className={form.formState.errors.schedule && 'text-red-500'}>
          زمان بندی
        </label>
        <Controller
          control={form.control}
          name="schedule"
          render={({ field }) => <Input {...field} type="date" name="schedule" id="schedule" />}
        />
      </div>

      <Button type="submit" variant="primary" disabled={form.formState.isSubmitting}>
        {form.formState.isSubmitting ? <Loader2 className="animate-spin" /> : 'انتشار محتوا'}
      </Button>
    </form>
  );
};
